...
def plot_can_duty_cycle(msg):  
    ...
    plot.plot(msg.values_0x93.time, msg.values_0x93.duty_cycle,'y.-', linewidth=0.5, label='motor_R-R(93h)_duty_cycle[%]')
    plot.plot(msg.values_0x94.time, msg.values_0x94.duty_cycle,'k.-', linewidth=0.5, label='motor_R-L(94h)_duty_cycle[%]')
    plot.plot(msg.values_0x81.time, msg.values_0x81.voltage,'b.-', linewidth=1, label='12V_voltage[V]') 
    plot.plot(msg.values_0x82.time, msg.values_0x82.voltage,'b.-', linewidth=1, label='42V_voltage[V]') 
...
def plot_desired_speed(msg):    
    plot_init(4)
    plot.plot(msg.desired_speed.time, msg.desired_speed.left,'r.-', linewidth=0.5, label='desired_speed.left[-]')
    plot.plot(msg.desired_speed.time, msg.desired_speed.right,'g.-', linewidth=0.5, label='desired_speed.right[-]')
    plot.title('desired_speed', size=12, loc='left')
    plot.xlabel('time[s]',size=10)
    plot.ylabel('desired_speed[-]',size=10)
    plot.legend(loc=1)
