...
pcan_file = r"C:\Users\xkadj\OneDrive\Plocha\ROBOTIKA\osgar_200121\osgar\K2_can.csv"
...
class MessageParser:
    def __init__(self, can_messages,robot):
        self.robot = robot
#        #extended can:
...
#        self.downdrops_rear = self.get_downdrops(downdrops_rear_messages)
    ...
    def get_can_angle(self,messages,message_ID):
        ...
        for row in range(len(messages)):
            if messages[row][2] == message_ID:
                angle = int(messages[row][3].zfill(16)[8:16],16)
                angle_2 = int(messages[row][3].zfill(16)[0:8],16)
#                right = int(messages[row][3].zfill(8)[12:16],16)
...
pcan_parsed = MessageParser(pcan_messages,robot)

k_plot.plot_can_duty_cycle(pcan_parsed,16)
k_plot.plot_can_duty_cycle_vesc(pcan_parsed,17)
k_plot.plot_can_tacho(pcan_parsed,18)
k_plot.plot_can_downdrops(pcan_parsed,19)
